...
for path in ("app.effects.passives", "app.services.passive_service"):
    try: pe = importlib.import_module(path); break
    except Exception: pass
if not pe: pytest.skip("passives module not found; adjust path")

def test_conflicting_passives_not_duplicated():
    fn = getattr(pe, "detect", None)
    if not fn: pytest.skip("Expected detect(loadout, target)")
