...
class TestItemRepositoryCaching(unittest.TestCase):
    def setUp(self):
        ...
        self.item_service.search_items_async = AsyncMock(
            return_value=[self.mock_items[0]]
        )
    ...
    def test_get_all_items_cached(self):
        first = asyncio.run(item_repository.get_all_items_async())
        second = asyncio.run(item_repository.get_all_items_async())
    ...
    def test_search_items(self):
        results = asyncio.run(item_repository.search_items_async("dragon"))
        self.item_service.search_items_async.assert_called_with("dragon", None)
...
class TestBossRepositoryCaching(unittest.TestCase):
    def setUp(self):
        ...
        self.boss_service.get_all_bosses_async = AsyncMock(
            return_value=self.mock_bosses
        )
        self.boss_service.get_boss_async = AsyncMock(return_value=self.mock_bosses[0])
        self.boss_service.search_bosses_async = AsyncMock(
            return_value=[self.mock_bosses[0]]
        )
    ...
    def test_get_all_bosses_cached(self):
        first = asyncio.run(boss_repository.get_all_bosses_async())
        second = asyncio.run(boss_repository.get_all_bosses_async())
    ...
    def test_search_bosses(self):
        results = asyncio.run(boss_repository.search_bosses_async("zul"))
        self.boss_service.search_bosses_async.assert_called_with("zul", None)
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]["id"], 1)
